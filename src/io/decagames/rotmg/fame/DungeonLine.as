...
package io.decagames.rotmg.fame
{
	...
	public class DungeonLine extends StatsLine
	{
		...
		override protected function setLabelsPosition():void
		{
			...
			var _local_2:TextureDataConcrete = ObjectLibrary.dungeonToPortalTextureData_[this.dungeonTextureName];
			if (_local_2)
			{
				_local_1 = _local_2.getTexture();
				_local_1 = TextureRedrawer.redraw(_local_1, 40, true, 0, false);
				this.dungeonBitmap = new Bitmap(_local_1);
				this.dungeonBitmap.x = (-(Math.round((_local_1.width / 2))) + 13);
				this.dungeonBitmap.y = (-(Math.round((_local_1.height / 2))) + 11);
