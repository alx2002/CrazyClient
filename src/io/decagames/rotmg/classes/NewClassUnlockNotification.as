	...
	public class NewClassUnlockNotification extends Sprite
	{
		...
		private function playAnimation():void
		{
			if ((!(this._timeLineMax)))
			{
				...
				this._timeLineMax.add(TweenMax.to(this._whiteSplash, 0.1, {"autoAlpha": 1, "transformAroundCenter": {"scaleX": 1, "scaleY": 1}, "ease": Bounce.easeOut}));
				this._timeLineMax.add(TweenMax.to(this._whiteSplash, 0.1, {"alpha": 0.4, "tint": 0, "ease": Expo.easeOut}));
				this._timeLineMax.add(TweenMax.to(this._contentContainer, 0.2, {"autoAlpha": 1, "transformAroundCenter": {"scaleX": 1, "scaleY": 1}, "ease": Bounce.easeOut}));
		...
		private function createCharacter():void
		{
			...
			;
			var _local_3:int = ObjectLibrary.playerChars_.length;
			var _local_4:int = this._objectTypes.shift();
			while (_local_5 < _local_3)
			{
				_local_1 = ObjectLibrary.playerChars_[_local_5];
				_local_2 = int(_local_1.@type);
				if (_local_4 == _local_2)
				{
					this._newClass = new Bitmap(this.getImageBitmapData(_local_1));
					break;
				}
				;
				_local_5++;
			...
			if (this._newClass)
			{
				...
				;
				this._characterName.text = _local_1.@id;
