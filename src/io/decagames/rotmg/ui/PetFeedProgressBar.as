...
package io.decagames.rotmg.ui
{
    ...
    public class PetFeedProgressBar extends UIGridElement 
    {
        ...
        private function render(_arg_1:int, _arg_2:int):void
        {
            ...
            var _local_9:* = (_arg_2 > _arg_1);
            var _local_10:int = (this._maxValue - AbilitiesUtil.abilityPowerToMinPoints(this._currentLevel));
            var _local_11:int = (_arg_1 - AbilitiesUtil.abilityPowerToMinPoints(this._currentLevel));
            var _local_12:Number = ((this._currentLevel == this._maxLevel) ? 1 : (_local_11 / _local_10));
            if (this._animateLevelProgress)
            {
            ...
            {
                this.drawProgress(_local_12);
            };
            if (_local_9)
            {
                if (_arg_2 >= this._maxValue)
                {
                    _local_5 = AbilitiesUtil.abilityPointsToLevel(_arg_2);
                    if (_local_5 >= this._maxLevel)
                    {
                        _local_5 = this._maxLevel;
                        this.drawSimulatedProgress(this._componentWidth, this.MAX_COLOR);
                        this.setLevelLabel(_local_5);
                    }
                    else
                    {
                        _local_6 = AbilitiesUtil.abilityPowerToMinPoints((_local_5 + 1));
                        _local_7 = AbilitiesUtil.abilityPowerToMinPoints(_local_5);
                        _local_4 = (_local_6 - _local_7);
                        _local_3 = (_arg_2 - _local_7);
                        this.drawSimulatedProgress((_local_3 / _local_4));
                        this.setLevelLabel(_local_5);
                    };
                }
                else
                {
                    _local_3 = (_arg_2 - AbilitiesUtil.abilityPowerToMinPoints(this._currentLevel));
                    _local_8 = (_local_3 / _local_10);
                    this.drawSimulatedProgress(_local_8, _local_12);
                };
            };
