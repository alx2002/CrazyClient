...
package io.decagames.rotmg.ui.gird
{
    ...
    public class UIGrid extends Sprite 
    {
        ...
        private function addDecorToRow(_arg_1:int, _arg_2:int, _arg_3:int):void
        {
            ...
            {
                _local_4 = TextureParser.instance.getSliceScalingBitmap("UI", this._decorBitmap);
                _local_4.x = Math.round((((_local_5 * (this.gridMargin / 2)) + ((_local_5 + 1) * (this.elementWidth + (this.gridMargin / 2)))) - (_local_4.width / 2)));
                _local_4.y = Math.round((((_arg_1 + _arg_2) - (_local_4.height / 2)) + (this.gridMargin / 2)));
                this.gridContent.addChild(_local_4);
                this.decors.push(_local_4);
                _local_5++;
            };
        }
        ...
        public function render():void
        {
            ...
            var _local_3:int = 1;
            var _local_7:int = int(int(Math.ceil((this.elements.length / this.numberOfColumns))));
            var _local_8:int = 1;
            for each (_local_1 in this.elements)
            {
                _local_1.resize(this.elementWidth);
                if (_local_1.height > _local_6)
                {
                    _local_6 = _local_1.height;
                };
                _local_1.x = _local_4;
                _local_1.y = _local_5;
                if (++_local_3 > this.numberOfColumns)
                {
                    if (this._decorBitmap != "")
                    {
                        _local_9 = _local_8;
                        this.addDecorToRow(_local_5, _local_6, (_local_3 - 1));
                    };
                    _local_8++;
                    _local_4 = 0;
                    if (((_local_8 == _local_7) && (this._centerLastRow)))
                    {
                        _local_2 = ((_local_8 * this.numberOfColumns) - this.elements.length);
                        _local_4 = int(int(Math.round((((_local_2 * this.elementWidth) + ((_local_2 - 1) * this.gridMargin)) / 2))));
                    };
                    _local_5 = (_local_5 + (_local_6 + this.gridMargin));
                    _local_6 = 0;
                    _local_3 = 1;
                }
                else
                {
                    _local_4 = (_local_4 + (this.elementWidth + this.gridMargin));
                };
            };
            if (((!(this._decorBitmap == "")) && (!(_local_9 == _local_8))))
            {
                this.addDecorToRow(_local_5, _local_6, (_local_3 - 1));
            };
