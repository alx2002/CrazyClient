...
package io.decagames.rotmg.ui
{
    ...
    public class ProgressBar extends UIGridElement 
    {
        ...
        private function render(_arg_1:int, _arg_2:int, _arg_3:Boolean):void
        {
            ...
            var _local_6:Number = ((this.componentWidth * _arg_1) / (this._maxValue - this._minValue));
            if (isNaN(_local_6))
            {
                _local_6 = 0;
            };
            ...
            else
            {
                ...
                this.progressShape.graphics.drawRect(0, 0, _local_6, this.componentHeight);
                this.progressShape.width = _local_6;
            };
            if (_local_5)
            {
                ...
                this.progressShape.graphics.drawRect(_local_6, 0, (((this.componentWidth * _arg_2) / (this._maxValue - this._minValue)) - _local_6), this.componentHeight);
            };
            var _local_7:String = this._dynamicLabelString.replace(DYNAMIC_LABEL_TOKEN, ((_local_5) ? _arg_2 : _arg_1));
            this._dynamicLabel.text = _local_7.replace(MAX_VALUE_TOKEN, this._maxValue);
            this._maxLabel.text = "";
            if (((_local_5) && (this._simulatedValueTextFormat)))
            {
                _local_4 = this._dynamicLabel.text.indexOf(_arg_2.toString());
                this._dynamicLabel.setTextFormat(this._simulatedValueTextFormat, _local_4, (_local_4 + _arg_2.toString().length));
