...
package io.decagames.rotmg.pets.components.petPortrait
{
	...
	public class PetPortrait extends Sprite
	{
		...
		private function startAnimation():void
		{
			...
			TweenLite.to(whiteRectangle, flashDuration, {"alpha": 1, "ease": Sine.easeIn, "onComplete": function():void
			{
				applyPetChange();
				...
				TweenLite.to(animationSpiral, spinDuration, {"transformAroundCenter": {"rotation": spinAngle}, "ease": Sine.easeOut});
				TweenLite.to(animationSpiral, hideDuration, {"alpha": 0, "delay": (spinDuration - 0.2), "overwrite": false, "ease": Sine.easeIn, "onComplete": function():void
				{
			...
			TweenLite.to(whiteRectangle, flashDuration, {"alpha": 0, "delay": flashDuration, "ease": Sine.easeOut, "overwrite": false});
		}
		...
		private function getMaxPointsFromAbilitiesList():int
		{
			...
			var _local_2:int;
			for each (_local_1 in this._petVO.abilityList)
			{
				if (_local_1.getUnlocked())
				{
					_local_2 = (_local_2 + AbilitiesUtil.abilityPowerToMinPoints(this._petVO.maxAbilityPower));
				}
				;
			}
			;
		...
		private function getCurrentPointsFromAbilitiesList():int
		{
			...
			for each (_local_1 in this._petVO.abilityList)
			{
				if (_local_1.getUnlocked())
				{
					_local_2 = (_local_2 + _local_1.points);
				}
				;
			}
			;
