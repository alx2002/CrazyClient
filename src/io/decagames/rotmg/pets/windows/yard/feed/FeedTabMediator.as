...
package io.decagames.rotmg.pets.windows.yard.feed
{
    ...
    public class FeedTabMediator extends Mediator 
    {
        ...
        {
            var _local_1:InventoryTile;
            var _local_2:int;
            var _local_3:FeedItem;
            this.view.clearGrid();
            this.items = new Vector.<FeedItem>();
            var _local_4:InventoryTabContent = this.hud.gameSprite.hudView.tabStrip.getTabView(InventoryTabContent);
            var _local_5:Vector.<InventoryTile> = new Vector.<InventoryTile>();
            if (_local_4)
            {
                _local_5 = _local_5.concat(_local_4.storage.tiles);
            };
            for each (_local_1 in _local_5)
            {
                _local_2 = _local_1.getItemId();
                if (((!(_local_2 == -1)) && (this.hasFeedPower(_local_2))))
                {
                    _local_3 = new FeedItem(_local_1);
                    this.items.push(_local_3);
                    this.view.addItem(_local_3);
                };
            };
        }
        ...
        private function refreshFeedPower():void
        {
            ...
            for each (_local_1 in this.items)
            {
                ...
                {
                    _local_2 = (_local_2 + _local_1.feedPower);
                    _local_3++;
                };
            };
            if (this.currentPet)
            {
                this.view.feedGoldButton.price = (FeedFuseCostModel.getFeedGoldCost(this.currentPet.rarity) * _local_3);
                this.view.feedFameButton.price = (FeedFuseCostModel.getFeedFameCost(this.currentPet.rarity) * _local_3);
                this.view.updateFeedPower(_local_2, this.currentPet.maxedAvailableAbilities());
            }
            else
            {
            ...
            this.simulateFeed.dispatch(_local_2);
        }
        ...
        {
            var _local_3:FeedItem;
            var _local_4:FeedPetRequestVO;
            var _local_5:SlotObjectData;
            ...
            var _local_6:Vector.<SlotObjectData> = new Vector.<SlotObjectData>();
            for each (_local_3 in this.items)
            {
                if (_local_3.selected)
                {
                    _local_5 = new SlotObjectData();
                    _local_5.objectId_ = _local_3.item.ownerGrid.owner.objectId_;
                    _local_5.objectType_ = _local_3.item.getItemId();
                    _local_5.slotId_ = _local_3.item.tileId;
                    _local_6.push(_local_5);
                };
            };
            this.currentPet.abilityUpdated.addOnce(this.abilityUpdated);
            this.showFade.dispatch();
            _local_4 = new FeedPetRequestVO(this.currentPet.getID(), _local_6, _arg_1);
