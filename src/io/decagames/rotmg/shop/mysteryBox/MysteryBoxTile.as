...
package io.decagames.rotmg.shop.mysteryBox
{
    ...
    public class MysteryBoxTile extends GenericBoxTile 
    {
        ...
        {
            var _local_1:int;
            var _local_2:UIItemContainer;
            var _local_4:int;
            var _local_3:Array = MysteryBoxInfo(_boxInfo).displayedItems.split(",");
            if (((_local_3.length == 0) || (MysteryBoxInfo(_boxInfo).displayedItems == "")))
            {
                return;
            };
            if (_infoButton)
            ...
            this.prepareResultGrid(_local_3.length);
            while (_local_4 < _local_3.length)
            {
                _local_2 = new UIItemContainer(_local_3[_local_4], 0, 0, this.resultElementWidth);
                this.displayedItemsGrid.addGridElement(_local_2);
                _local_4++;
            };
        }
