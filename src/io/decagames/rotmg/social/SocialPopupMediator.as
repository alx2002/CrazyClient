...
package io.decagames.rotmg.social
{
    ...
    public class SocialPopupMediator extends Mediator 
    {
        ...
        {
            var _local_2:Vector.<FriendVO>;
            var _local_3:FriendVO;
            var _local_4:Vector.<FriendVO>;
            var _local_5:int;
            var _local_6:int;
            var _local_7:int;
            var _local_8:* = (!(_arg_1 == ""));
            ...
            {
                _local_4 = this.socialModel.getAllInvitations();
                this.view.addFriendCategory((("Invitations (" + _local_4.length) + ")"));
                _local_5 = ((_local_4.length > SocialPopupView.MAX_VISIBLE_INVITATIONS) ? SocialPopupView.MAX_VISIBLE_INVITATIONS : _local_4.length);
                _local_6 = 0;
                while (_local_6 < _local_5)
                {
                    this.view.addInvites(new FriendListItem(_local_4[_local_6], SocialItemState.INVITE));
                    _local_6++;
                };
            ...
            _local_2 = ((_local_8) ? this.socialModel.getFilterFriends(_arg_1) : this.socialModel.friendsList);
            this.view.addFriendCategory((((("Friends (" + this.socialModel.numberOfFriends) + "/") + SocialConfig.MAX_FRIENDS) + ")"));
            for each (_local_3 in _local_2)
            {
                _local_7 = ((_local_3.isOnline) ? SocialItemState.ONLINE : SocialItemState.OFFLINE);
                this.view.addFriend(new FriendListItem(_local_3, _local_7));
            };
        ...
        {
            var _local_1:Vector.<GuildMemberVO>;
            var _local_2:int;
            var _local_3:int;
            var _local_4:GuildMemberVO;
            var _local_5:int;
            var _local_6:GuildVO = this.socialModel.guildVO;
            var _local_7:String = ((_local_6) ? _local_6.guildName : "No Guild");
            var _local_8:int = ((_local_6) ? _local_6.guildTotalFame : 0);
            this.view.addGuildInfo(new GuildInfoItem(_local_7, _local_8));
            if (((_local_6) && (this.socialModel.numberOfGuildMembers > 0)))
            {
                this.view.addGuildCategory((((("Guild Members (" + this.socialModel.numberOfGuildMembers) + "/") + 50) + ")"));
                _local_1 = _local_6.guildMembers;
                _local_2 = _local_1.length;
                _local_3 = 0;
                while (_local_3 < _local_2)
                {
                    _local_4 = _local_1[_local_3];
                    _local_5 = ((_local_4.isOnline) ? SocialItemState.ONLINE : SocialItemState.OFFLINE);
                    this.view.addGuildMember(new GuildListItem(_local_4, _local_5, _local_6.myRank));
                    _local_3++;
                };
