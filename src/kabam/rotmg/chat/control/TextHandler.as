...
package kabam.rotmg.chat.control
{
    ...
    public class TextHandler 
    {
        ...
        public function execute(_arg_1:Text):void
        {
            ...
            var _local_6:String;
            var _local_7:Array;
            var _local_8:Vector.<Boolean>;
            var _local_9:int;
            var _local_10:Array;
            var _local_11:int;
            var _local_12:Boolean = (_arg_1.numStars_ == -1);
            var _local_13:Player = this.hudModel.gameSprite.map.player_;
            if (((((!(Parameters.data_.chatAll)) && (!(_arg_1.name_ == this.model.player.name_))) && (!(_local_12))) && (!(this.isSpecialRecipientChat(_arg_1.recipient_)))))
            {
            ...
            if (((!(_local_12)) && (_arg_1.name_.indexOf("#") == -1)))
            {
            ...
            _local_4 = _arg_1.text_.toLowerCase();
            for each (_local_3 in Parameters.data_.wordNotiList)
            {
                if (((((!(_local_4.indexOf(_local_3) == -1)) && (!(!(_local_4.indexOf("call") == -1)))) && (!(!(_local_4.indexOf("event") == -1)))) && (Parameters.data_.wordNoti)))
                {
                    _local_13.levelUpEffect("", true);
            ...
            {
                _local_5 = "server.dungeon_opened_by";
                _local_4 = _arg_1.text_;
                if (_local_4.indexOf(_local_5) != -1)
                {
            ...
            {
                for each (_local_5 in Parameters.data_.eventCalls)
                {
                    if (_local_4.indexOf(_local_5) != -1)
                    {
                        switch (_local_5)
                        {
                            case "shtrs_Defense_System.new":
                                _local_5 = "Avatar";
                                SoundEffectLibrary.play("level_up");
                                _local_13.notifyPlayer(_local_5, 0xFFFF, 3000);
                                _local_5 = "";
                                break;
                            case "Dragon_Head_Leader.new":
                                _local_5 = "Rock Dragon";
                                SoundEffectLibrary.play("level_up");
                                _local_13.notifyPlayer(_local_5, 0xFFFF, 3000);
                                _local_5 = "";
                                break;
                            case "LH_Lost_Sentry.new":
                                _local_5 = "Lost Sentry";
                                SoundEffectLibrary.play("level_up");
                                _local_13.notifyPlayer(_local_5, 0xFFFF, 3000);
                                _local_5 = "";
                                break;
                            case "Temple_Encounter.new":
                                _local_5 = "Temple Statues";
                                SoundEffectLibrary.play("level_up");
                                _local_13.notifyPlayer(_local_5, 0xFFFF, 3000);
                                _local_5 = "";
                                break;
                        ...
                        if (_local_5 != "")
                        {
                            _local_5 = _local_5.slice(0, -4);
                            _local_5 = _local_5.split("_").join(" ");
                            SoundEffectLibrary.play("level_up");
                            _local_13.notifyPlayer(_local_5, 0xFFFF, 3000);
                            break;
                        };
            ...
            if (_arg_1.recipient_)
            {
                ...
                else
                {
					...
					switch (args[1]) {
                        ...
                        _local_4 = _arg_1.name_.toLowerCase();
                        if (_arg_1.text_ == "s?")
                        {
                            if (this.isLocalFriend(_local_4))
                            {
                        ...
                        if (_arg_1.text_.substring(0, 2) == "g=")
                        {
                            if (this.isLocalFriend(_local_4))
                            {
                                _local_7 = _arg_1.text_.match("g=(\\d{1,8})$");
                                _local_8 = new <Boolean>[false, false, false, false, false, false, false, false, false, false, false, false];
                                _local_9 = 4;
                                while (_local_9 < 12)
                                {
                                    if (_local_7[1].substr((_local_9 - 4), 1) == "1")
                                    {
                                        _local_8[_local_9] = true;
                                    };
                                    _local_9++;
                                };
                                GameServerConnectionConcrete.receivingGift = _local_8;
                        ...
                        if (_arg_1.text_.substring(0, 2) == "s=")
                        {
                            if (this.isLocalFriend(_local_4))
                            {
                                _local_10 = _arg_1.text_.substr(2).split(" ");
                                PlayGameCommand.curip = _local_10[0];
                                GameServerConnectionConcrete.sRec = true;
                                GameServerConnectionConcrete.whereto = _local_10[1];
                            };
                        };
                    };
                };
            };
            _local_2 = _arg_1.text_;
            if (((_local_2.length > 19) && (_local_2.substr(7, 12) == "NexusPortal.")))
            {
                _local_2 = (_local_2.substr(0, 7) + _local_2.substr(19));
            };
            _arg_1.text_ = this.replaceIfSlashServerCommand(_local_2);
            if (((_local_12) && (this.isToBeLocalized(_local_2))))
            {
                _local_2 = this.getLocalizedString(_local_2, _local_13);
            };
            if (Parameters.data_.AutoReply)
            {
                ...
                {
                    _local_2 = this.getSplinterReply(_arg_1.text_);
                    if (_local_2 != "")
                    {
                        this.hudModel.gameSprite.gsc_.playerText(_local_2);
                    };
                };
            };
            ...
            for each (_local_6 in Parameters.data_.tptoList)
            {
            ...
            _local_11 = Parameters.timerPhaseTimes[_arg_1.text_];
            if (_local_11 > 0)
            {
                Parameters.timerActive = true;
                Parameters.phaseChangeAt = (getTimer() + _local_11);
                Parameters.phaseName = Parameters.timerPhaseNames[_arg_1.text_];
            };
            if (((_arg_1.objectId_ >= 0) && ((_arg_1.numStars_ > Parameters.data_.chatStarRequirement) || (_local_12))))
            {
                this.showSpeechBaloon(_arg_1, _local_2);
            };
            if (((_local_12) || ((this.account.isRegistered()) && ((!(Parameters.data_.hidePlayerChat)) || (this.isSpecialRecipientChat(_arg_1.name_))))))
            {
        ...
        {
            var lb:LineBuilder;
            var text:Text = _arg_1;
            var message:ChatMessage = _arg_2;
            try
            {
