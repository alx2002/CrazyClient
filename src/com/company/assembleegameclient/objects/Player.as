	...
	public class Player extends Character
	{
		...
		public function getFamePortrait(_arg_1:int):BitmapData
		{
			...
			if (this.famePortrait_ == null)
			{
				...
				_arg_1 = int(int(((4 / _local_2.image_.width) * _arg_1)));
		...
		public function getFameBonus():int
		{
			...
			var _local_3:int;
			var _local_4:uint;
			while (_local_4 < GeneralConstants.NUM_EQUIPMENT_SLOTS)
			{
				if (((equipment_) && (equipment_.length > _local_4)))
				{
					_local_1 = equipment_[_local_4];
					if (_local_1 != -1)
					{
						_local_2 = ObjectLibrary.xmlLibrary_[_local_1];
						if (((!(_local_2 == null)) && (_local_2.hasOwnProperty("FameBonus"))))
						{
							_local_3 = (_local_3 + int(_local_2.FameBonus));
						}
						;
					}
					;
		...
		public function calculateStatBoosts():void
		{
			...
			while (_local_6 < GeneralConstants.NUM_EQUIPMENT_SLOTS)
			{
				if (((equipment_) && (equipment_.length > _local_6)))
				{
					_local_1 = equipment_[_local_6];
					if (_local_1 != -1)
					{
						_local_2 = ObjectLibrary.xmlLibrary_[_local_1];
						if (((!(_local_2 == null)) && (_local_2.hasOwnProperty("ActivateOnEquip"))))
						{
							for each (_local_3 in _local_2.ActivateOnEquip)
							{
								if (_local_3.toString() == "IncrementStat")
								{
									_local_4 = int(_local_3.@stat);
		...
		public function lootItem(_arg_1:int, _arg_2:Container, _arg_3:int, _arg_4:int, _arg_5:int):void
		{
			...
			var _local_7:Boolean = ((_arg_4 == HEALTH_POT) || (_arg_4 == MAGIC_POT));
			...
			{
				if ((((_local_6) && (_local_7)) && (!((nextLootSlot == HEALTH_POT) || (nextLootSlot == MAGIC_POT)))))
				{
		...
		public function autoloot_(_arg_1:int):void
		{
			...
			;
			var _local_6:Vector.<Container> = this.getLootBags();
			_local_6 = this.getLootableBags(_local_6, MAX_LOOT_DIST);
			for each (_local_2 in _local_6)
			{
				_local_5 = 0;
				for each (_local_3 in _local_2.equipment_)
				{
					if (((this.isWantedItem(_local_3)) && (this.okToLoot(_local_3))))
					{
						if (_local_2.objectId_ != GameServerConnectionConcrete.ignoredBag)
						{
							this.lootItem(this.nextAvailableInventorySlotMod(), _local_2, _local_5, _local_3, _arg_1);
						}
						;
						return;
					}
		...
		override public function update(_arg_1:int, _arg_2:int):Boolean
		{
			...
			if (this == map_.player_)
			{
				...
				;
				if ((((((this == this.map_.player_) && (!(this.map_.gs_.isSafeMap))) && (Parameters.data_.autoAbil)) && (!(this.equipment_[1] == -1))) && (!(isSilenced()))))
				{
					this.autoAbility();
				}
				;
		...
		public function useAltWeapon_(_arg_1:Number, _arg_2:Number, _arg_3:int, _arg_4:Boolean = false):Boolean
		{
			...
			if (_arg_3 == UseType.START_USE)
			{
				...
				;
				_local_7 = int(_local_11.MpCost);
				if (_local_7 > this.mp_)
				{
				...
				{
					_local_8 = (Number(_local_11.Cooldown) * 1000);
				}
				;
				this.nextAltAttack_ = (_local_6 + _local_8);
				...
				{
					_local_9 = Math.atan2(((_arg_4) ? Number((_arg_2 - this.y_)) : Number(_arg_2)), ((_arg_4) ? Number((_arg_1 - this.x_)) : Number(_arg_1)));
					this.doShoot(_local_6, _local_10, _local_11, (((_arg_4) ? 0 : Parameters.data_.cameraAngle) + _local_9), false);
			...
			else
			{
				if (_local_11.hasOwnProperty("MultiPhase"))
				{
					...
					_local_7 = int(_local_11.MpEndCost);
					if (_local_7 <= this.mp_)
					{
						_local_9 = Math.atan2(((_arg_4) ? Number((_arg_2 - this.y_)) : Number(_arg_2)), ((_arg_4) ? Number((_arg_1 - this.x_)) : Number(_arg_1)));
						this.doShoot(_local_6, _local_10, _local_11, (((_arg_4) ? 0 : Parameters.data_.cameraAngle) + _local_9), false);
		...
		public function sbAssist(_arg_1:int, _arg_2:int):void
		{
			...
			var _local_7:Number;
			var _local_8:XML;
			var _local_9:int = this.equipment_[1];
			if (_local_9 == -1)
			{
				return;
			}
			;
			var _local_10:XML = ObjectLibrary.xmlLibrary_[_local_9];
			for each (_local_8 in _local_10.Activate)
			{
				if (_local_8.toString() == ActivationType.TELEPORT)
				{
