...
package com.company.assembleegameclient.ui.tooltip
{
	...
	public class ClassToolTip extends ToolTip
	{
		...
		private function createCharacter():void
		{
			...
			var _local_3:int = int(int(((4 / _local_2.width()) * 100)));
		...
		private function createEquipmentTypes():void
		{
			...
			var _local_5:Vector.<int> = ConversionUtil.toIntVector(this._playerXML.SlotTypes);
			var _local_6:Vector.<int> = ConversionUtil.toIntVector(this._playerXML.Equipment);
			while (_local_7 < EquipmentUtil.NUM_SLOTS)
			{
				_local_1 = _local_6[_local_7];
				if (_local_1 > -1)
				{
					_local_2 = ObjectLibrary.getRedrawnTextureFromType(_local_1, 40, true);
					_local_3 = new Bitmap(_local_2);
					_local_3.x = (_local_7 * 22);
					_local_3.y = -12;
					this._equipmentContainer.addChild(_local_3);
				}
				else
				{
					_local_4 = EquipmentUtil.getEquipmentBackground(_local_5[_local_7], 2);
					if (_local_4)
					{
						_local_4.x = (12 + (_local_7 * 22));
						_local_4.filters = FilterUtil.getDarkGreyColorFilter();
						this._equipmentContainer.addChild(_local_4);
					}
					;
				}
				;
		...
		private function createQuestText():void
		{
			...
			if (this._nextStarFame > 0)
			{
				...
				{
					this.nextClassQuest_.setStringBuilder(new LineBuilder().setParams("Earn {nextStarFame} Fame with {typeToDisplay} to unlock the next Star", {"nextStarFame": this._nextStarFame, "typeToDisplay": getDisplayId(this._playerXML)}));
				}
				else
				{
		...
		private function createStarProgress():void
		{
			...
			var _local_11:Vector.<int> = FameUtil.STARS;
			var _local_12:int = _local_11.length;
			while (_local_13 < _local_12)
			{
				_local_2 = _local_11[_local_13];
				_local_3 = ((this._charStats != null) ? this._charStats.bestFame() : 0);
				_local_4 = ((_local_3 >= _local_2) ? 0xFF00 : 16549442);
				_local_5 = (_local_13 < this._numberOfStars);
				_local_6 = (20 + (_local_13 * 10));
				_local_7 = new StarGraphic();
				_local_7.x = (_local_10 + ((_local_6 - _local_7.width) / 2));
				_local_7.transform.colorTransform = ((_local_5) ? FULL_STAR : EMPTY_STAR);
				this._progressContainer.addChild(_local_7);
				_local_8 = new UILabel();
				_local_8.text = _local_2.toString();
				DefaultLabelFormat.characterToolTipLabel(_local_8, _local_4);
				_local_8.x = (_local_10 + ((_local_6 - _local_8.width) / 2));
				_local_8.y = 14;
				this._progressContainer.addChild(_local_8);
				_local_1.beginFill(0x1C1C1C);
				_local_1.drawRect(_local_10, 31, _local_6, 4);
				if (_local_3 > 0)
				{
					_local_1.beginFill(_local_4);
					...
					else
					{
						if (_local_13 == 0)
						{
							_local_9 = int(int(((_local_3 / _local_2) * _local_6)));
							_local_1.drawRect(_local_10, 31, _local_9, 4);
						}
						else
						{
							if (_local_3 > _local_11[(_local_13 - 1)])
							{
								_local_9 = int(int((((_local_3 - _local_11[(_local_13 - 1)]) / (_local_2 - _local_11[(_local_13 - 1)])) * _local_6)));
								_local_1.drawRect(_local_10, 31, _local_9, 4);
							}
							;
						}
				...
				;
				_local_10 = (_local_10 + (1 + _local_6));
				_local_13++;
			}
			;
		}
		...
		private function createClassUnlocks():void
		{
			...
			var _local_7:int = ObjectLibrary.playerChars_.length;
			var _local_8:Vector.<XML> = ObjectLibrary.playerChars_;
			var _local_9:String = this._playerXML.@id;
			var _local_10:int = ((this._charStats != null) ? this._charStats.bestLevel() : 0);
			while (_local_12 < _local_7)
			{
				_local_1 = _local_8[_local_12];
				_local_2 = _local_1.@id;
				if (((!(_local_9 == _local_2)) && (_local_1.UnlockLevel)))
				{
					for each (_local_3 in _local_1.UnlockLevel)
					{
						if (_local_9 == _local_3.toString())
						{
							_local_4 = int(_local_3.@level);
							_local_5 = ((_local_10 >= _local_4) ? 0xFF00 : 0xFF0000);
							_local_6 = new UILabel();
							_local_6.text = ((("Reach level " + _local_4.toString()) + " to unlock ") + _local_2);
							DefaultLabelFormat.characterToolTipLabel(_local_6, _local_5);
							_local_6.y = _local_11;
							this._classUnlockContainer.addChild(_local_6);
		...
		private function createUnlockRequirements():void
		{
			...
			var _local_4:AppendingLineBuilder = new AppendingLineBuilder();
			for each (_local_1 in this._playerXML.UnlockLevel)
			{
				_local_2 = ObjectLibrary.idToType_[_local_1.toString()];
				_local_3 = int(_local_1.@level);
				if (this._playerModel.getBestLevel(_local_2) < int(_local_1.@level))
				{
					_local_4.pushParams(TextKey.TO_UNLOCK_REACH_LEVEL, {"unlockLevel": _local_3, "typeToDisplay": ObjectLibrary.typeToDisplayId_[_local_2]});
				}
				;
			}
			;
