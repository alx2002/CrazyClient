...
package com.company.assembleegameclient.ui.tooltip
{
	...
	public class EquipmentToolTip extends ToolTip
	{
		...
		private function addProjectile(_arg_1:XML, _arg_2:XML = null):void
		{
			...
			{
				this.effects.push(new Effect(TextKey.EFFECT_FOR_DURATION, {"effect": _local_3, "duration": _local_3.@duration}).setColor(TooltipHelper.NO_DIFF_COLOR));
			}
			;
		}
			...
			for each (activateXML in this.objectXML.Activate)
			{
				...
				if (this.comparisonResults.processedTags[activateXML.toXMLString()] != true)
				{
					...
					case ActivationType.COND_EFFECT_AURA: 
						...
						this.effects.push(new Effect(TextKey.EFFECT_FOR_DURATION, {"effect": activateXML.@effect, "duration": activateXML.@duration}).setColor(TooltipHelper.NO_DIFF_COLOR));
						break;
					case ActivationType.COND_EFFECT_SELF: 
						this.effects.push(new Effect(TextKey.EFFECT_ON_SELF, {"effect": ""}));
						this.effects.push(new Effect(TextKey.EFFECT_FOR_DURATION, {"effect": activateXML.@effect, "duration": activateXML.@duration}));
						break;
					case ActivationType.STAT_BOOST_SELF: 
						this.effects.push(new Effect("{amount} {stat} for {duration} ", {"amount": this.prefix(activateXML.@amount), "stat": new LineBuilder().setParams(StatData.statToName(int(activateXML.@stat))), "duration": TooltipHelper.getPlural(activateXML.@duration, "second")}));
						break;
					case ActivationType.HEAL: 
						this.effects.push(new Effect(TextKey.INCREMENT_STAT, {"statAmount": (("+" + activateXML.@amount) + " "), "statName": new LineBuilder().setParams(TextKey.STATUS_BAR_HEALTH_POINTS)}));
						break;
					case ActivationType.HEAL_NOVA: 
						if (((activateXML.hasOwnProperty("@damage")) && (int(activateXML.@damage) > 0)))
						{
							this.effects.push(new Effect("{damage} damage within {range} sqrs", {"damage": activateXML.@damage, "range": activateXML.@range}));
						}
						;
						this.effects.push(new Effect(TextKey.PARTY_HEAL, {"effect": new AppendingLineBuilder().pushParams(TextKey.HP_WITHIN_SQRS, {"amount": activateXML.@amount, "range": activateXML.@range}, TooltipHelper.getOpenTag(TooltipHelper.NO_DIFF_COLOR), TooltipHelper.getCloseTag())}));
						break;
					case ActivationType.MAGIC: 
						this.effects.push(new Effect(TextKey.INCREMENT_STAT, {"statAmount": (("+" + activateXML.@amount) + " "), "statName": new LineBuilder().setParams(TextKey.STATUS_BAR_MANA_POINTS)}));
						break;
					...
					case ActivationType.GENERIC_ACTIVATE: 
						...
						;
						tokens = {"range": activateXML.@range, "effect": activateXML.@effect, "duration": activateXML.@duration};
					...
					case ActivationType.STAT_BOOST_AURA: 
						...
						tokens2 = {"range": activateXML.@range, "stat": statStr, "amount": activateXML.@amount, "duration": activateXML.@duration};
		...
		private function getBulletCreate(_arg_1:XML, _arg_2:XML = null):void
		{
			...
			this.effects.push(new Effect(_local_7, {"numShots": TooltipHelper.compare(_local_3.a, _local_3.b), "angle": TooltipHelper.getPlural(_local_4.a, "degree"), "minDistance": TooltipHelper.compareAndGetPlural(_local_5.a, _local_5.b, "square", false), "maxDistance": TooltipHelper.compareAndGetPlural(_local_6.a, _local_6.b, "square")}));
		}
		...
		private function getSkull(_arg_1:XML, _arg_2:XML = null):void
		{
			...
			var _local_9:int = int(int(((_local_6 / 10) * _local_8)));
			...
			;
			this.effects.push(new Effect(_local_16, {"damage": TooltipHelper.compare(_local_11.a, _local_11.b), "radius": TooltipHelper.compare(_local_12.a, _local_12.b), "heal": TooltipHelper.compare(_local_14.a, _local_14.b), "ignoreDef": TooltipHelper.compare(_local_15.a, _local_15.b), "healRange": TooltipHelper.compare(MathUtil.round(_local_13.a, 2), MathUtil.round(_local_13.b, 2))}));
		...
		private function getTrap(_arg_1:XML, _arg_2:XML = null):void
		{
			...
			this.effects.push(new Effect(_local_10, {"damage": TooltipHelper.compare(_local_3.a, _local_3.b), "radius": TooltipHelper.compare(_local_4.a, _local_4.b)}));
			this.AddConditionToEffects(_arg_1, _arg_2, "Slowed", 5);
			this.effects.push(new Effect("{throwTime} to arm for {duration} ", {"throwTime": TooltipHelper.compareAndGetPlural(_local_6.a, _local_6.b, "second", false), "duration": TooltipHelper.compareAndGetPlural(_local_5.a, _local_5.b, "second")}));
		...
		private function getLightning(_arg_1:XML, _arg_2:XML = null):void
		{
			...
			var _local_5:ComPair = new ComPair(_arg_1, _arg_2, "decrDamage", 0);
			var _local_6:int = this.GetIntArgument(_arg_1, "wisPerTarget", 10);
			var _local_7:int = this.GetIntArgument(_arg_1, "wisDamageBase", _local_5.a);
			var _local_8:int = this.GetIntArgument(_arg_1, "wisMin", 50);
			var _local_9:int = Math.max(0, (_local_4 - _local_8));
			var _local_10:int = int(int((_local_9 / _local_6)));
			var _local_11:int = int(int(((_local_7 / 10) * _local_9)));
			var _local_12:ComPair = new ComPair(_arg_1, _arg_2, "maxTargets");
			_local_12.add(_local_10);
			var _local_13:ComPair = new ComPair(_arg_1, _arg_2, "totalDamage");
			_local_13.add(_local_11);
			var _local_14:String = this.colorUntiered("Lightning: ");
			_local_14 = (_local_14 + (("{targets}" + this.colorWisBonus(_local_10)) + " targets\n"));
			_local_14 = (_local_14 + (("{damage}" + this.colorWisBonus(_local_11)) + " damage"));
			if (_local_5.a)
			{
				if (_local_5.a < 0)
				{
				...
				;
				_local_14 = (_local_14 + ((", " + _local_3) + " by \n{decrDamage} for each subsequent target"));
			}
			;
			this.effects.push(new Effect(_local_14, {"targets": TooltipHelper.compare(_local_12.a, _local_12.b), "damage": TooltipHelper.compare(_local_13.a, _local_13.b), "decrDamage": TooltipHelper.compare(_local_5.a, _local_5.b, false, "", _local_15)}));
		...
		private function getDecoy(_arg_1:XML, _arg_2:XML = null):void
		{
			...
			;
			this.effects.push(new Effect(_local_10, {"duration": TooltipHelper.compareAndGetPlural(_local_3.a, _local_3.b, "second"), "angleOffset": TooltipHelper.compareAndGetPlural(_local_4.a, _local_4.b, "degree"), "distance": TooltipHelper.compareAndGetPlural(_local_6.a, _local_6.b, "square"), "travelTime": TooltipHelper.compareAndGetPlural(_local_7, _local_8, "second"), "numShots": TooltipHelper.compare(_local_9.a, _local_9.b)}));
		}
		...
		private function getPoison(_arg_1:XML, _arg_2:XML = null):void
		{
			...
			this.effects.push(new Effect(_local_10, {"totalDamage": TooltipHelper.compare(_local_3.a, _local_3.b, true, "", false, (!(this.sameActivateEffect))), "radius": TooltipHelper.compareAndGetPlural(_local_4.a, _local_4.b, "square", true, (!(this.sameActivateEffect))), "impactDamage": TooltipHelper.compare(_local_7.a, _local_7.b, true, "", false, (!(this.sameActivateEffect))), "duration": TooltipHelper.compareAndGetPlural(_local_5.a, _local_5.b, "second", false, (!(this.sameActivateEffect)))}));
			this.AddConditionToEffects(_arg_1, _arg_2, "Nothing", 5);
		...
		{
			var _local_5:ComPair;
			var _local_6:String;
			var _local_7:String = ((_arg_1.hasOwnProperty("@condEffect")) ? _arg_1.@condEffect : _arg_3);
			if (_local_7 != "Nothing")
			{
				_local_5 = new ComPair(_arg_1, _arg_2, "condDuration", _arg_4);
				if (_arg_2)
				{
					_local_6 = ((_arg_2.hasOwnProperty("@condEffect")) ? _arg_2.@condEffect : _arg_3);
					if (_local_6 == "Nothing")
					{
						_local_5.b = 0;
					}
				...
				;
				this.effects.push(new Effect("Inflicts {condition} for {duration} ", {"condition": _local_7, "duration": TooltipHelper.compareAndGetPlural(_local_5.a, _local_5.b, "second")}));
			}
			;
		}
		...
		private function getComparedStatText(_arg_1:XML):Object
		{
			...
			return ({"statAmount": (this.prefix(_local_3) + " "), "statName": new LineBuilder().setParams(StatData.statToName(_local_2))});
		}
		...
		private function getComparedStatColor(_arg_1:XML):uint
		{
			var match:XML;
			var otherAmount:int;
			var otherMatches:XMLList;
			var activateXML:XML = _arg_1;
		...
		private function buildRestrictionsLineBuilder():StringBuilder
		{
			...
			for each (_local_1 in this.restrictions)
			{
				...
				_local_5.pushParams(_local_1.text_, {"unUsableClass": _local_4, "usableClasses": this.getUsableClasses(), "keyCode": KeyCodes.CharCodeStrings[Parameters.data_.useSpecial]}, _local_2, _local_3);
			}
			;
			return (_local_5);
		}
