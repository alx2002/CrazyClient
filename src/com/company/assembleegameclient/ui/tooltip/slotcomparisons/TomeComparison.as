	...
	public class TomeComparison extends SlotComparison
	{
		...
		override protected function compareSlots(_arg_1:XML, _arg_2:XML):void
		{
			...
			if (((nova.length() == 1) && (otherNova.length() == 1)))
			{
				...
				{
					comparisonStringBuilder.pushParams(TooltipHelper.wrapInFontTag("{damage} damage within {range} sqrs", ("#" + TooltipHelper.NO_DIFF_COLOR.toString(16))), {"damage": nova.@damage, "range": nova.@range});
				}
				;
				...
				innerStringBuilder = new LineBuilder().setParams(TextKey.HP_WITHIN_SQRS, {"amount": amount.toString(), "range": range.toString()}).setPrefix(TooltipHelper.getOpenTag(getTextColor((wavg - otherWavg)))).setPostfix(TooltipHelper.getCloseTag());
			...
			else
			{
				if (itemXML.@id == "Tome of Holy Protection")
				{
					...
					comparisonStringBuilder.pushParams(TextKey.EFFECT_FOR_DURATION, {"effect": TextKey.wrapForTokenResolution(TextKey.ACTIVE_EFFECT_ARMORED), "duration": tag.@duration}, TooltipHelper.getOpenTag(UNTIERED_COLOR), TooltipHelper.getCloseTag());
					processedTags[tag.toXMLString()] = true;
				}
				;
			}
