...
package com.company.assembleegameclient.ui.tooltip.slotcomparisons
{
	...
	public class SealComparison extends SlotComparison
	{
		...
		override protected function compareSlots(_arg_1:XML, _arg_2:XML):void
		{
			...
			if (this.canCompare())
			{
				...
				if (itemXML.@id == "Seal of Blasphemous Prayer")
				{
					...
					comparisonStringBuilder.pushParams(TextKey.EFFECT_FOR_DURATION, {"effect": TextKey.wrapForTokenResolution(TextKey.ACTIVE_EFFECT_INVULERABLE), "duration": tag.@duration}, TooltipHelper.getOpenTag(UNTIERED_COLOR), TooltipHelper.getCloseTag());
		...
		private function handleHealingText():void
		{
			...
			_local_8.pushParams(TextKey.EFFECT_FOR_DURATION, {"effect": TextKey.wrapForTokenResolution(TextKey.ACTIVE_EFFECT_HEALING), "duration": _local_1.toString()}, TooltipHelper.getOpenTag(_local_7), TooltipHelper.getCloseTag());
		...
		private function handleDamagingText():void
		{
			...
			_local_8.pushParams(TextKey.EFFECT_FOR_DURATION, {"effect": TextKey.wrapForTokenResolution(TextKey.ACTIVE_EFFECT_DAMAGING), "duration": _local_1.toString()}, TooltipHelper.getOpenTag(_local_7), TooltipHelper.getCloseTag());
			comparisonStringBuilder.pushParams(TextKey.PARTY_EFFECT, {"effect": _local_8});
