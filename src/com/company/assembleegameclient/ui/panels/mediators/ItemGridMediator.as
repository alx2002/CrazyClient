...
package com.company.assembleegameclient.ui.panels.mediators
{
    ...
    public class ItemGridMediator extends Mediator 
    {
        ...
        private function onTileMove(_arg_1:ItemTileEvent):void
        {
            ...
            var _local_3:XML;
            var _local_4:XML;
            var _local_5:InteractiveItemTile = _arg_1.tile;
            if (this.swapTooSoon())
            {
                _local_5.resetItemPosition();
                return;
            };
            var _local_6:* = DisplayHierarchy.getParentWithTypeArray(_local_5.getDropTarget(), InteractiveItemTile, Map, CooldownTimer);
            if (((_local_5.getItemId() == PotionInventoryModel.HEALTH_POTION_ID) || (_local_5.getItemId() == PotionInventoryModel.MAGIC_POTION_ID)))
            {
                this.onPotionMove(_arg_1);
                return;
            };
            if ((_local_6 is InteractiveItemTile))
            {
                _local_2 = (_local_6 as InteractiveItemTile);
                if (this.canSwapItems(_local_5, _local_2))
                {
                    this.swapItemTiles(_local_5, _local_2);
                };
            }
            else
            {
                if (((_local_6 is Map) || (this.hudModel.gameSprite.map.mouseX < 300)))
                {
                    this.dropItem(_local_5);
                }
                else
                {
                    if ((_local_6 is CooldownTimer))
                    {
                        _local_3 = ObjectLibrary.xmlLibrary_[_local_5.itemSprite.itemId];
                        _local_4 = ObjectLibrary.xmlLibrary_[this.view.curPlayer.equipment_[1]];
                        if (_local_3.SlotType == _local_4.SlotType)
                        {
                            GameServerConnection.instance.invSwap(this.view.curPlayer, _local_5.ownerGrid.owner, _local_5.tileId, _local_5.itemSprite.itemId, this.view.curPlayer, 1, this.view.curPlayer.equipment_[1]);
                        };
                    };
                };
            };
        ...
        private function petFoodCancel(_arg_1:InteractiveItemTile):Function
        {
            var itemSlot:InteractiveItemTile = _arg_1;
