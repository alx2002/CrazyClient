		...
		override public function draw(_arg_1:Camera, _arg_2:int):void
		{
			...
			while (_local_25 <= _local_22)
			{
				...
				while (_local_6 <= _local_24)
				{
					...
					if (_local_3 != null)
					{
						_local_11 = (_local_19.x - _local_3.center_.x);
						_local_12 = (_local_19.y - _local_3.center_.y);
						_local_13 = ((_local_11 * _local_11) + (_local_12 * _local_12));
						if (_local_13 <= _arg_1.maxDistSq_)
						{
			...
			for each (_local_4 in goDict_)
			{
				...
				if ((!((_local_4.dead_) || (_local_4.size_ == 0))))
				{
			...
			{
				_local_14 = ((Parameters.BREATH_THRESH - player_.breath_) / Parameters.BREATH_THRESH);
				_local_15 = (Math.abs(Math.sin((_arg_2 / 300))) * 0.75);
				BREATH_CT.alphaMultiplier = (_local_14 * _local_15);
			...
			{
				_local_7 = this.getFilterIndex();
				_local_8 = StaticInjectorContext.getInjector().getInstance(Render3D);
				_local_8.dispatch(this.graphicsData_, this.graphicsData3d_, width_, height_, _arg_1, _local_7);
				_local_9 = 0;
				while (_local_9 < this.graphicsData_.length)
				{
					if (((this.graphicsData_[_local_9] is GraphicsBitmapFill) && (GraphicsFillExtra.isSoftwareDraw(GraphicsBitmapFill(this.graphicsData_[_local_9])))))
					{
						this.graphicsDataStageSoftware_.push(this.graphicsData_[_local_9]);
						this.graphicsDataStageSoftware_.push(this.graphicsData_[(_local_9 + 1)]);
						this.graphicsDataStageSoftware_.push(this.graphicsData_[(_local_9 + 2)]);
					}
					else
					{
						if (((this.graphicsData_[_local_9] is GraphicsSolidFill) && (GraphicsFillExtra.isSoftwareDrawSolid(GraphicsSolidFill(this.graphicsData_[_local_9])))))
						{
							this.graphicsDataStageSoftware_.push(this.graphicsData_[_local_9]);
							this.graphicsDataStageSoftware_.push(this.graphicsData_[(_local_9 + 1)]);
							this.graphicsDataStageSoftware_.push(this.graphicsData_[(_local_9 + 2)]);
			...
			if (((!(player_ == null)) && (!((player_.condition_[ConditionEffect.CE_FIRST_BATCH] & ConditionEffect.MAP_FILTER_BITMASK) == 0))))
			{
				...
				if (player_.isDrunk())
				{
					...
					_local_10.push(new BlurFilter(_local_16, _local_16));
				}
				;
				if (player_.isBlind())
				{
					_local_10.push(BLIND_FILTER);
				}
				;
				map_.filters = _local_10;
			}
			else
			{
