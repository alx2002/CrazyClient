...
package com.company.assembleegameclient.background
{
	...
	public class StarBackground extends Background
	{
		...
		{
			var _local_1:Star;
			var _local_2:ImageSet = AssetLibrary.getImageSet("stars");
			var _local_3:Star = new Star(((Math.random() * 1000) - 500), ((Math.random() * 1000) - 500), (4 * (0.5 + (0.5 * Math.random()))), _local_2.images_[int((_local_2.images_.length * Math.random()))]);
			for each (_local_1 in this.stars_)
			{
				if (PointUtil.distanceXY(_local_3.x_, _local_3.y_, _local_1.x_, _local_1.y_) < 3)
				{
					return;
				}
				;
