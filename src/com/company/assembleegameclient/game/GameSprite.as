...
package com.company.assembleegameclient.game
{
	...
	public class GameSprite extends AGameSprite
	{
		...
		override public function initialize():void
		{
			...
			var _local_4:Object = {"game_net_user_id": _local_1.gameNetworkUserId(), "game_net": _local_1.gameNetwork(), "play_platform": _local_1.playPlatform()};
		...
		private function showNewsUpdate(_arg_1:Boolean = true):void
		{
			...
			var _local_3:ILogger = StaticInjectorContext.getInjector().getInstance(ILogger);
			var _local_4:NewsModel = StaticInjectorContext.getInjector().getInstance(NewsModel);
			_local_3.debug("NEWS UPDATE -- making button");
			if (_local_4.hasValidModalNews())
			{
				_local_3.debug("NEWS UPDATE -- making button - ok");
		...
		private function updateNearestInteractive():void
		{
			...
			var _local_2:IInteractiveObject;
			...
			for each (_local_1 in map.goDict_)
			{
				_local_2 = (_local_1 as IInteractiveObject);
				if (((_local_2) && ((!(_local_2 is Pet)) || (this.map.isPetYard))))
				{
					if (((Math.abs((_local_7 - _local_1.x_)) < GeneralConstants.MAXIMUM_INTERACTION_DISTANCE) || (Math.abs((_local_8 - _local_1.y_)) < GeneralConstants.MAXIMUM_INTERACTION_DISTANCE)))
					{
						_local_4 = PointUtil.distanceXY(_local_1.x_, _local_1.y_, _local_7, _local_8);
						if (((_local_4 < GeneralConstants.MAXIMUM_INTERACTION_DISTANCE) && (_local_4 < _local_6)))
						{
							_local_6 = _local_4;
							_local_3 = _local_2;
						}
		...
		private function onEnterFrame(_arg_1:Event):void
		{
			...
			if (mui_.held)
			{
				_local_6 = (WebMain.STAGE.mouseX - mui_.heldX);
				Parameters.data_.cameraAngle = (mui_.heldAngle + (_local_6 * (Math.PI / 180)));
				if (((!(Options.hidden)) && (Parameters.data_.tiltCam)))
				{
					_local_5 = (WebMain.STAGE.mouseY - mui_.heldY);
					mui_.heldY = WebMain.STAGE.mouseY;
					this.camera_.nonPPMatrix_.appendRotation(_local_5, Vector3D.X_AXIS, null);
				}
				;
			}
			;
			...
			if (this.focus)
			{
				camera_.configureCamera(this.focus, ((_local_8) ? Boolean(_local_8.isHallucinating()) : false));
			...
			if (_local_8 != null)
			{
				...
				;
				moveRecords_.addRecord(_local_3, _local_8.x_, _local_8.y_);
			}
			;
